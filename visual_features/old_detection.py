...
from visual_baseline import load_model_and_transform, ALLOWED_MODELS
from utils import compute_last_layer_size
from data import BBoxDataset
from bounding_box_utils import compute_iou
...
class Detector(nn.Module):
    def __init__(self,
        ...
        self.backbone, self.transform = load_model_and_transform(
            Namespace(device=device, model_name=backbone_name),
            keep_pooling=backbone_name != 'clip-rn',
            add_flatten=False
        )

        # excluding Normalize bc. is implemented in FastRCNN and cannot be bypassed
        tmean, tstd = self.transform.transforms[-1].mean, self.transform.transforms[-1].std
        self.transform = torchvision.transforms.Compose(self.transform.transforms[:-1])
        ...
        self.backbone.out_channels = compute_last_layer_channels(self.backbone)
        ...
        self.model = FasterRCNN(
            ...
            num_classes=nr_target_categories, min_size=224, max_size=224,
    ...
    def process_batch(self, batch, loss_fn):
        ...
        x = x.to(self.device)
        targets = [
            {'boxes': y['bbox'][i].unsqueeze(0).to(self.device), 'labels': y['target'][i].unsqueeze(0).to(self.device)}
        ...
        if self.training:
            # training
            yps = None
            total_loss = sum(list(res.values()))
        else:
            # validation
            yps = res
            total_loss = -1.0
        gts = torch.cat([y['target'].view(-1, 1), y['bbox']], dim=-1).to(self.device)
        return total_loss, yps, gts
"""
...
def get_data(args, obj_dict=None, model_transform=None):
    # Transforms should be not none because the models require PIL images

    def collate(batch):
        x = torch.stack([el[0] for el in batch], dim=0)
        y = {
            'target': torch.cat([el[1]['labels'] for el in batch], dim=0),
            'bbox': torch.cat([el[1]['boxes'] for el in batch], dim=0)
        }
        return x, y

    train_set = BBoxDataset(os.path.join(args.data_path, 'train'), transform=model_transform, object_conversion_table=obj_dict)
    valid_set = BBoxDataset(os.path.join(args.data_path, 'valid'), transform=model_transform, object_conversion_table=obj_dict)
    test_set = BBoxDataset(os.path.join(args.data_path, 'test'), transform=model_transform, object_conversion_table=obj_dict)
    dataset = {'train': train_set, 'valid': valid_set, 'test': test_set}
    train_dl = torch.utils.data.DataLoader(train_set, args.batch_size, num_workers=4, collate_fn=collate)
    valid_dl = torch.utils.data.DataLoader(valid_set, args.batch_size, num_workers=4, collate_fn=collate)
...
def run_training(args):
    ...
    full_dataset, train_dl, valid_dl = get_data(args, model_transform=transform)
    loss = get_loss(args)
...
def check_dataset():
    ...
    batch_size = 3
    ds = BBoxDataset('../bboxes/test')
    dl = torch.utils.data.DataLoader(ds, batch_size=batch_size)
    smp = next(iter(dl))
    net = ConditionalRegressor(len(ds.get_object_set()), device='cpu')
    l_fn = lambda x, y: 0.0
    pprint(net.process_batch(smp, l_fn))
...
__default_train_config__ = {
    ...
    'regressor': ('conditional', ['detector', 'conditional']),
...
def freeze_exp(args):
    models = ALLOWED_MODELS - {'all', 'untrained-rn-zero-init'}
...
if __name__ == "__main__":
    ...
    for name, arg in __default_train_config__.items():
        if type(arg) == bool:
    ...
    if args.regressor == 'detector':
        if input("Warning: this file is deprecated for the regressor type 'Detector'. Continue anyway? [[y]/n]") not in ['y', 'Y', 'yes']:
            exit(0)

    if args.check_dataset:
        check_dataset()
    elif args.freeze_unfreeze_exp:
        freeze_exp(args)
    elif args.train:
        if args.use_wandb:
